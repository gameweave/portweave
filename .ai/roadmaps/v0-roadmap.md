...
**Depends on:** —
**Gameweave prior art:** none — pattern documented in [.claude/rules/error-handling.md](../../.claude/rules/error-handling.md).
...
**Gameweave prior art:** Gameweave's internal worktree-port module — the 8-service shape Portweave config must be able to express.
...
- Gameweave's internal git-detection helper — `git rev-parse --show-toplevel`, `--git-common-dir`, `worktree list --porcelain`
- Gameweave's internal namespace-derivation helper — `main` vs slug-hash, env override
...
- Hash format: Gameweave uses 8-char hash of absolute path. Match exactly, or upgrade to a longer/different scheme? Recommend **match Gameweave** for migration debugging parity.

**Invocation:**

```
/create-feature worktree-context — Implement git worktree detection + namespace derivation matching Gameweave's internal worktree-context git-detection and namespace-derivation helpers (PORTWEAVE_* env overrides instead of GAMEWEAVE_*). Falls back to cwd for non-git dirs. Produces the AllocationKey used by the registry. Depends on Feature 1.
```
...
**Parity rows:** §7.2 #2, #7.
**Depends on:** #1
**Gameweave prior art:** Gameweave's internal file-locked registry helper — the canonical pattern (mkdir-based lock dir, 100 retries × 25ms, 30s stale-lock TTL, dedup by offset, prune-missing-worktrees on read).
...
- Locking: directory-mutex via `fs.mkdir` on `registry.lock/` with retry + exponential backoff + stale-lock cleanup (configurable but default to Gameweave's 30s).
...
- Stale-lock TTL — match Gameweave's 30s, or shorter? Gameweave's value is battle-tested; **use 30s**.

**Invocation:**

```
/create-feature registry-storage — Implement ~/.config/portweave/registry.json I/O with directory-mutex locking (mirror Gameweave's internal file-locked registry helper pattern: 100 retries × 25ms, 30s stale-lock TTL, atomic temp+rename writes, prune-on-read for deleted worktrees). No offset field — record schema is { key, ports, lastUsedAt, namespace }. Depends on Feature 1.
```
...
**Gameweave prior art:** Gameweave's internal worktree-port module (shape only — Portweave does NOT use the `base + offset*100` formula).
...
- Gameweave's internal env-injection helper — the env-injection + URL-construction logic
- Gameweave's internal E2E port-env helper — the additional URL vars for E2E
...
**Depends on:** #2, #3, #5, #6
**Gameweave prior art:** Gameweave's internal dev-orchestration wrapper — the orchestrating wrapper.
...
**Gameweave prior art:** none directly — but conceptually mirrors how Gameweave's internal env-injection helper could be called in-process.
...
### 10. `parity-verification` — Gameweave drop-in adoption acceptance test
...
- Author a `portweave.config.json` declaring Gameweave's 8 services (api, ws, vite, dynamodb, dynamodb-admin, kinesis, kinesis-tls, ses) with their env-var names and URL templates per Gameweave's internal env-injection helper.
- Integration test that simulates a Gameweave-like environment: two simulated worktrees, runs `portweave run -- <noop>` in each, asserts that:
  - All 8 services receive ports
  - URL templates resolve identically to what Gameweave's internal env-injection helper would emit
...
- `__tests__/gameweave-parity.test.ts` (root-level cross-cutting integration test, per [.claude/rules/testing.md](../../.claude/rules/testing.md))
- `examples/gameweave.config.json` (sample config that Gameweave would use)
- README.md migration section

**Acceptance criteria sketch:**

- [ ] All 14 Gameweave parity items from DESIGN.md §7.2 verified explicitly in the test suite
...
- Should this also run Gameweave's real e2e suite in CI, or is the simulated test enough at v0? Recommend simulated only at v0 — real e2e adds a heavy CI dependency on Gameweave's repo state.

**Invocation:**

```
/create-feature parity-verification — Add the Gameweave drop-in acceptance test per DESIGN.md §7.3: portweave.config.json for Gameweave's 8 services, integration test verifying all 14 §7.2 parity rows, README migration steps. Final v0 acceptance gate. Depends on all prior features.
```
...
cp /path/to/portweave/examples/gameweave.config.json portweave.config.json
...
Plus the Gameweave-side validation per DESIGN.md §7.3 — a separate task once v0 ships.
