...
- Gameweave's worktree-ports module (modeled on Gameweave's internal
  worktree-port system) defines the 8-service shape (api, app/vite, authApi,
  dynamodb, dynamodbAdmin, kinesis, kinesisTls, ses, ws) that Portweave's
  config schema must be expressive enough to describe, including the
  Kinesis-plain-and-TLS pairing that motivates the `group` key. Gameweave
