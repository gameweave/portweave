...
Gameweave's parity story leans on constructed env vars like
...
- Gameweave's worktree-env application (modeled on Gameweave's internal
  worktree-port system) inspires both halves of the v0 surface: the
  `setIfUnset`-style env injection that lets existing process env (and
  seeded `.env` values) win over computed defaults, and the URL-construction
  pattern (`http://localhost:${port}`, `ws://localhost:${port}`) that
  Portweave generalizes into per-service `discoveryEnv` templates. Gameweave
...
- Gameweave's E2E port-env helper inspires the additional URL vars surfaced
  specifically for E2E consumers (`VITE_API_URL`, `VITE_WS_URL`,
  `E2E_API_ORIGIN`). In Portweave these aren't a separate code path; they're
  just additional entries in a service's `discoveryEnv` map, computed by the
  same template-evaluation pass.
