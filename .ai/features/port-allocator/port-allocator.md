...
1. **Two simultaneous worktrees of the same repo must never receive
   ...
   block. Gameweave's per-project offset model already solves this within
...
2. **Unrelated projects on the same machine must never collide on
   overlapping ports.** This is the model change versus Gameweave
...
- Gameweave's worktree-ports module (modeled on Gameweave's internal
  worktree-port system) inspires the _per-service shape_ only: a worktree's
  allocation is a map of named services to port numbers, all derived
  together so they move as a unit. **Portweave does NOT use Gameweave's
  `base + offset*100` formula.** That formula is a per-project offset
  multiplier with a hard 99-offset cap and no cross-project collision
  protection. Portweave replaces it with block selection from a machine-wide
  pool, with live conflict probing on each candidate port. Gameweave's module
  is design inspiration for the per-service result shape, not source imported
  at runtime.
