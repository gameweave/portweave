...
v0 ships only if it can actually replace boardflip's existing worktree-port
...
the integrated system behaves the way boardflip's hand-rolled stack does today.

This feature is the single test gate that proves it. It exercises Portweave
end-to-end against a `portweave.config.json` that mirrors boardflip's eight
...
what `apply-worktree-env.ts` emits today — including the constructed
discovery URLs that boardflip's runtime depends on. It also ships the
migration documentation a boardflip maintainer (or any future adopter)
...
- [config-loader](../config-loader/config-loader.md) — the test authors a
  real `portweave.config.json` mirroring boardflip's eight services with
...
- [env-resolution](../env-resolution/env-resolution.md) — the test compares
  ...
  against what boardflip's `apply-worktree-env.ts` would emit for the same
...
- [run-command](../run-command/run-command.md) — the simulation invokes
  ...
  path a real boardflip user would hit, not an in-process shortcut.
- [show-command](../show-command/show-command.md) — the test confirms the
  ...
  parity that boardflip never had but Portweave's adopters get for free.
...
- [reference/boardflip/scripts/src/utils/apply-worktree-env.ts](../../../reference/boardflip/scripts/src/utils/apply-worktree-env.ts)
  — the env-var contract this test verifies parity with. The file defines
  the complete set of env vars boardflip seeds onto a worktree's child
  process (`API_PORT`, `VITE_API_PORT`, `WS_PORT`, `VITE_WS_PORT`,
  `VITE_PORT`, `AUTH_API_PORT`, `VITE_AUTH_PORT`, `DYNAMODB_PORT`,
  `DYNAMODB_ENDPOINT`, `KINESIS_PORT`, `KINESIS_TLS_PORT`,
  ...
  asserts that the resulting environment matches what
  `apply-worktree-env.ts` would emit for the same port numbers — modulo the
  `GAMEWEAVE_*` → `PORTWEAVE_*` namespace rename captured in DESIGN.md
  §7.2 row 8. The file is read-only design context; nothing under
...
- Author a `portweave.config.json` (kept under `examples/`) that declares
  boardflip's eight services — `api`, `ws`, `vite`, `dynamodb`,
  ...
  endpoints in `apply-worktree-env.ts`.
- A root-level cross-cutting integration test at
  `__tests__/boardflip-parity.test.ts` that simulates a boardflip-like
...
- Explicit assertions in the test suite covering each of the 14 rows in
  ...
  feature spec.
- A README section documenting the boardflip migration steps following
  DESIGN.md §7.3 — deleting boardflip's per-project allocation helpers,
...
- Running boardflip's real end-to-end suite in CI against a live
  Portweave binary. See the open question below — recommended deferred for
  v0; the simulated test is the gate for now.
- Verifying boardflip-specific runtime behaviors that live outside
  ...
  boardflip's responsibility on its own side of the integration boundary.
- A general-purpose parity-test framework for other future adopters.
  Boardflip is the v0 reference consumer; if more adopters arrive, the
...
- All 14 boardflip parity items from DESIGN.md §7.2 are verified explicitly
...
- For each allocated block, the resolved env-var map and constructed
  discovery URLs match what boardflip's
  [reference/boardflip/scripts/src/utils/apply-worktree-env.ts](../../../reference/boardflip/scripts/src/utils/apply-worktree-env.ts)
  would emit for the same port numbers — modulo the
  `GAMEWEAVE_*` → `PORTWEAVE_*` namespace rename from §7.2 row 8.
...
- The migration documentation in the README is followable by a fresh
  ...
  change in a boardflip-style consumer.
...
- Should this also run boardflip's real e2e suite in CI, or is the
  simulated test enough at v0? Recommend simulated only at v0 — real e2e
  adds a heavy CI dependency on boardflip's repo state.
