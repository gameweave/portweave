...
DESIGN.md §7.2 row **#12** — _Wrapper CLI entry point_ — boardflip's
...
- [reference/boardflip/scripts/bin/dev.ts](../../../reference/boardflip/scripts/bin/dev.ts)
  — inspires the _orchestration shape_ of the run command: resolve
  worktree context, compute ports, seed env from dotenv, apply env,
  print a human-readable startup summary, then `spawn` the child with
  `stdio: 'inherit'` and propagate the exit code. Boardflip hardwires
  PM2 as the child and the boardflip-specific service list into the
  ...
  anonymous mode) declared. Boardflip's signal handling is implicit
...
- PM2-style multi-process orchestration. Boardflip's `dev.ts` happens
