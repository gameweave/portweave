...
Two simultaneous consumers from one code path. The boardflip parity goal ([DESIGN.md §7.2 rows 5, 6, 9](../../DESIGN.md)) requires both — boardflip's [apply-worktree-env.ts](../../../reference/boardflip/scripts/src/utils/apply-worktree-env.ts) hardcodes service names and URL shapes; Portweave lifts both into declarative config so the same machinery serves any project.
