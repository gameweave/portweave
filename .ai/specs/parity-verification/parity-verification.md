...
**Decision-log rows:** [#7](../../decision-log.md) (v0 scope cap is Gameweave parity), [#14](../../decision-log.md) (drop-in adoption by Gameweave is the v0 verification criterion), every prior feature row by transitive reference

## Problem

v0 ships only if Portweave can replace Gameweave's hand-rolled worktree-port system without users noticing functional regressions ([decision-log row #14](../../decision-log.md), [DESIGN.md §7.3](../../DESIGN.md)). Every prior v0 feature spec verifies its own slice in isolation — config parses, registry locks, allocator avoids collisions, env resolution produces the right map, the CLI orchestrates, the library facade returns the same allocation. None of those, individually, prove that the **integrated system** behaves like Gameweave's stack does today.
...
1. Authors a `portweave.config.json` mirroring Gameweave's eight services with their env-var names, group labels, and `discoveryEnv` URL templates.
2. Simulates two real git worktrees of the same repo and invokes `portweave run -- <noop>` in each.
3. Asserts that every one of the 14 Gameweave parity items in [DESIGN.md §7.2](../../DESIGN.md) holds against the integrated v0 system.
4. Documents the migration steps a Gameweave maintainer follows to retire their hand-rolled system, written so a fresh reader can execute the procedure without external context.

This is also where the env-var contract Portweave produces gets cross-checked against the exact shape Gameweave's internal env-injection helper emits today — including the constructed discovery URLs (`WEBSOCKET_ENDPOINT`, `DYNAMODB_ENDPOINT`, `KINESIS_ENDPOINT`, `SES_ENDPOINT`) that Gameweave's runtime depends on.
...
### `examples/gameweave.config.json` — the parity config

Authored to exactly match Gameweave's env-var surface. The eight services with their grouping and discovery URLs:
...
This config is the exemplar shipped with Portweave and the primary input to the integration test. The discovery-URL shapes mirror Gameweave's internal env-injection helper — `VITE_API_PORT` and `VITE_WS_PORT` are deliberate aliases of `API_PORT` / `WS_PORT` (Gameweave's `seedIfMissing` pattern), and the four `*_ENDPOINT` URLs use `http://localhost:${...}` even for kinesis-TLS-adjacent services because Gameweave itself does. Faithful parity, not improved parity.

### `__tests__/gameweave-parity.test.ts` — the integration gate
...
2. Create `tmpdir/main/` as the main worktree, write `examples/gameweave.config.json` content into `tmpdir/main/portweave.config.json`.
...
Append a `## Migrating from a hand-rolled worktree-port system (Gameweave)` section to [README.md](../../../README.md) covering the six steps from [DESIGN.md §7.3](../../DESIGN.md):

1. Delete `scripts/src/utils/worktree-context-*.ts` and related helpers in the Gameweave consumer.
2. Delete `packages/shared/src/worktree-ports.ts`.
3. Add `portweave.config.json` declaring the eight services with their env-var names and URL templates (cross-link to `examples/gameweave.config.json` as a concrete reference).
...
6. Acceptance: all Gameweave e2e tests still pass; worktree behavior identical from a user's POV.

Section is written so a fresh reader who has never seen Portweave can execute the procedure against a real Gameweave checkout without external context. Cite the parity test in this repo as evidence that the integrated system already passes the contract a Gameweave user is relying on.
...
This feature's test lives at `__tests__/gameweave-parity.test.ts`. The supporting fixture (the build-the-CLI helper, the noop child source, the use-runtime.mjs consumer) lives at `__tests__/fixtures/gameweave-parity/` — kept under the test directory so `structure:check` doesn't need to be relaxed.
...
- v0 acceptance: all 14 Gameweave parity rows verified by `__tests__/gameweave-parity.test.ts`. Gameweave migration documented in README. The acceptance test is the gate for shipping v0; merging without it green is not permitted.

## Acceptance criteria

- [ ] `examples/gameweave.config.json` exists and declares all eight services (`api`, `ws`, `vite`, `dynamodb`, `dynamodb-admin`, `kinesis`, `kinesis-tls`, `ses`) with their env-var names, group labels (`dynamodb`, `kinesis`), and `discoveryEnv` URL templates matching Gameweave's internal env-injection helper shape.
...
- [ ] `__tests__/gameweave-parity.test.ts` exists and runs a two-worktree simulation against a real temp git repo, with `XDG_CONFIG_HOME` scoped to a test-only registry path.
...
- [ ] `README.md` contains a `## Migrating from a hand-rolled worktree-port system (Gameweave)` section covering the six steps from DESIGN.md §7.3, cross-linking to `examples/gameweave.config.json`, written so a fresh reader can execute the procedure without external context.
...
- [ ] One decision-log row is appended on `Status: shipped` capturing that all 14 Gameweave parity rows are verified by `__tests__/gameweave-parity.test.ts` and that the test is the v0 ship gate.

## Open questions

- **Live Gameweave e2e in CI.** Per the feature doc's open question and the roadmap's recommendation, this spec stays with the **simulated** test only at v0. The recommendation: ratify this as a v0 decision and defer the live-Gameweave-e2e-in-CI variant to a post-v0 follow-up if real adoption surfaces gaps the simulation misses. The cost of running Gameweave's full e2e against a Portweave binary in CI is high (Gameweave repo checkout, its own dependency install, its e2e harness setup) and the value at v0 is incremental — the simulated test covers every contractual surface. Flag in case approval wants to flip this; if so, the spec adds a CI workflow file under `.github/workflows/gameweave-e2e.yml` that's documented as nightly rather than per-PR to avoid blocking development.
