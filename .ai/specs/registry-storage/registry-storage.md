...
Per [decision-log row #4](../../decision-log.md), the record has **no `offset` field**. The registry diverges from boardflip's reference at exactly this point. The on-disk file is JSON:
...
The `key` shape is what Feature #3 (worktree-context) produces. Carrying `namespace` both inside `key` and at the top of the record is intentional — the key is what identifies a worktree; the top-level `namespace` is the value PM2 / log consumers read out (DESIGN.md §7.2 row 4, [reference/boardflip/scripts/src/utils/worktree-context-namespace.ts](../../../reference/boardflip/scripts/src/utils/worktree-context-namespace.ts)). Feature #3 owns the derivation; this feature only stores what it's given.
...
Mirrors [reference/boardflip/scripts/src/utils/worktree-context-registry.ts:102–140](../../../reference/boardflip/scripts/src/utils/worktree-context-registry.ts). The atomic primitive is `fs.mkdir(lockDir)` — POSIX guarantees `mkdir` of an existing directory fails with `EEXIST`, so the create-or-fail race is resolved by the kernel. Implementation pins:

- **Retry budget:** `LOCK_RETRY_COUNT = 100`, `LOCK_RETRY_DELAY_MS = 25`. Worst-case wait ≈ 2.5s. Matches boardflip exactly so debugging is the same.
- **Stale-lock TTL:** `STALE_LOCK_MS = 30_000`. If the existing lock directory's `mtimeMs` is older than 30s, the next retry first removes it (`fs.rm(lockDir, { force: true, recursive: true })`), then loops. Matches boardflip.
- **Sleep primitive:** use `setTimeout` via `await new Promise((r) => setTimeout(r, LOCK_RETRY_DELAY_MS))` rather than boardflip's `Atomics.wait` SharedArrayBuffer trick — Portweave is async-first (Vite/Next config call sites in Feature #9 are async, so the registry path must be async-friendly), and the SharedArrayBuffer trick is a workaround for sync code paths boardflip needed but Portweave doesn't.
