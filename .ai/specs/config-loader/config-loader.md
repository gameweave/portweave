...
- `group` — optional non-empty string. Services sharing a `group` value are allocated as a contiguous block by Feature 5 (DESIGN.md §7.2 row #10; motivated by Kinesis 4567/4568 in Gameweave's internal worktree-port system).
...
- Validates `count` is an integer in `[1, 100]`. Out-of-range or non-integer returns `err(PortweaveError(CONFIG_INVALID, ...))` with a message naming the constraint. Upper bound of 100 prevents accidental pool exhaustion at the allocator layer and matches Gameweave's de-facto 99-offset historical cap (DESIGN.md §4: "99-offset cap → no cap" applies to the per-worktree pool, but a single anonymous invocation asking for hundreds of ports is almost certainly a bug).
