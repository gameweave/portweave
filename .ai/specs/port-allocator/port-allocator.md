...
Three source files plus a tests directory under `src/allocator/`. The public surface for downstream features (env-resolution, run-command, library-runtime) is a single `allocate(key, config)` entry point that handles lookup-based reuse, block search, and registry mutation in one call.

> **Revised by [decision-log #37](../../decision-log.md) (2026-06-03):** the reuse path is now **unconditional** — it does not probe the stored ports or evict on a "taken" result. The TCP-listen probe runs **only** on the fresh-allocation path. Step 3 below and the "Live-conflict reuse invalidation" acceptance criterion describe the original (superseded) behavior; the "Reuse path: probe vs. trust" open question that flagged this is now resolved. See decision-log #37 for the full rationale.
...
3. **Lookup-and-reuse (reuse path).** Find an existing entry by `keysEqual(entry.key, key)`. If found, call `handle.touch(key)` to bump `lastUsedAt` and return `ok({ allocation: entry-with-fresh-lastUsedAt, reused: true })` — **unconditionally, without probing the entry's ports** ([decision-log #37](../../decision-log.md)). A port bound by this key's own services is the normal runtime state, not a conflict, and a loopback probe cannot distinguish the two; reusing as-is keeps repeated `allocate()` / `ports()` / `env()` / `allocation()` calls for the same key idempotent. Stale entries are dropped by age-based pruning in `withRegistry` before this step runs, not by probing here.

   _Superseded behavior (pre–decision-#37):_ the original design probed every stored port and, on any `'taken'` result, `handle.remove(key)`'d the entry and fell through to fresh allocation. That broke idempotency the moment the caller's own services were bound.
...
- [ ] Idempotent reuse while ports are bound (revised per [decision-log #37](../../decision-log.md); supersedes the original "live-conflict reuse invalidation" criterion): a second `allocate` for the same key returns the **existing** block with `reused: true` even when one or more of its ports are currently bound (on `127.0.0.1` or `0.0.0.0`), verified by `src/allocator/__tests__/allocate.test.ts` using a real `net.Server`; the public runtime reads (`ports()` / `env()` / `allocation()`) are likewise stable across repeated calls while a port is bound, verified by `src/runtime/__tests__/index.test.ts`. The probe-and-evict-on-reuse behavior is removed; the probe is retained only on the fresh-allocation path (next criterion).
...
- **Reuse path: probe vs. trust. → RESOLVED 2026-06-03 ([decision-log #37](../../decision-log.md)):** flipped to trust-the-registry — reuse is now **unconditional** (no probe, no eviction); the probe is retained only for fresh allocation. Real usage surfaced the friction exactly as this question anticipated (a `portweave/runtime` config consumer reallocated to a different block after its own sibling services were already bound). Original analysis, now superseded: The reuse-after-probe-fail behavior treats _any_ taken port as invalidating the cached allocation, including the case where the user's _own_ dev server (from a prior `portweave run` in the same worktree) is currently bound. The alternative — trust the registry, let the dev server fail to bind, surface the conflict on bind error — is closer to Gameweave's behavior (Gameweave has no probe at all) and avoids surprise port-rotation when re-running while a server is up. The current spec follows the feature doc literally ("same ports as long as those ports remain free"), but the trade-off is real. Flagging in case the approval pass wants to flip this; if so, the probe-on-reuse step becomes "probe only the ports that don't match the stored entry's owner" — which we cannot determine without a marker file or pid record in the entry, expanding the registry schema. Recommend keeping the spec as written and revisiting only if real usage surfaces the friction.
