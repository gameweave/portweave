...
1. **Two simultaneous worktrees of the same repo never overlap.** Boardflip's per-project offset solved this within one project; Portweave preserves it under a machine-wide model.
2. **Unrelated projects on the same machine never collide.** This is the model change versus boardflip ([DESIGN.md §5.1](../../DESIGN.md), [§7.2 row 14](../../DESIGN.md)). Two repos both defaulting to Vite 5173 must come up cleanly side-by-side.
...
- **Direction:** ascending from `range.start`. Predictable, debuggable, matches boardflip's "low offsets first" intuition.
