// Duplicated, structurally-identical mirror of the backend `PanelSnapshot`
// contract (src/panel/types.ts). The frontend toolchain is isolated from src/,
// so it cannot cleanly import the backend type across the project boundary.
// Drift is caught by the backend server contract test, which asserts the served
// JSON's shape. See .ai/specs/management-panel/02-frontend.md ("Type sharing").
