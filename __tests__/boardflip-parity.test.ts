/**
 * Boardflip drop-in acceptance gate — §7.2 parity verification.
 *
 * This is the v0 ship gate. Every one of the 14 boardflip parity items in
...
const BOARDFLIP_CONFIG_PATH = (() => {
  const thisFile = new URL(import.meta.url).pathname
  const projectRoot = path.resolve(path.dirname(thisFile), '..')
  return path.join(projectRoot, 'examples', 'boardflip.config.json')
})()
...
async function createFixture(): Promise<TestFixture> {
  ...
  const configContent = fs.readFileSync(BOARDFLIP_CONFIG_PATH, 'utf8')
  fs.writeFileSync(
...
async function loadBoardflipConfig(): Promise<LoadedConfig> {
  const runtimeIndexPath = resolveRuntimeIndexPath()
  ...
  const examplesDir = path.dirname(BOARDFLIP_CONFIG_PATH)
  // Pass explicit configPath because the file is boardflip.config.json, not portweave.config.json
  const result = await loadConfig(examplesDir, {
    configPath: BOARDFLIP_CONFIG_PATH,
  })
...
async function testConfigLoaderValidation(): Promise<void> {
  const cfg = await loadBoardflipConfig()
...
describe('boardflip parity — §7.2 acceptance gate', { timeout: 60000 }, () => {
...
// ---------------------------------------------------------------------------
// examples/boardflip.config.json — config loader validation
// ---------------------------------------------------------------------------

describe('examples/boardflip.config.json — config loader validation', () => {
